    ...
    <form @submit.prevent="submitHandler()" novalidate>
        <div class="inputBox">
            <label class="custom-field">
                ...
                <span class="text-danger" v-if="v$.phone.minLength.$invalid || v$.phone.maxLength.$invalid">Пароль должен состоять из 11 символов</span>
                <span class="text-danger" v-if="v$.phone.numeric.$invalid && !v$.phone.minLength.$invalid && !v$.phone.maxLength.$invalid">Пароль должен состоять из цифр</span>
            </label>
        </div>
        <div class="submitBtn">
...
export default {
  ...
  validations() {
    return{
      phone: {required, maxLength: maxLength(11), minLength: minLength(11), numeric},
    }
  },
  methods: {
    async submitHandler() {
      this.v$.$touch()
        ...
        await axios.post('api/auth/reg/phone/sms-auth/send/', { 
            phone: this.phone
        }).then(
            response => {
                localStorage.setItem('phone' , this.phone)
                console.log(response)
                this.$router.push('/registration/checkphone')
            }
        ).catch(
            error => {
                console.log(error)
