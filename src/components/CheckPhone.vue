    ...
    <form @submit.prevent="submitHandler()" novalidate>
        <div class="inputBox">
            <label class="custom-field">
                <input style="width: 440px;" type="text" v-model.trim="checkCode" @blur="v$.checkCode.$touch()" :class="{'is-invalid': v$.checkCode.$error, 'valid': !v$.checkCode.$error}">
                <span class="placeholder">Код из смс</span>
                <span class="text-danger" v-if="!v$.checkCode.$model && v$.checkCode.$dirty">Это поле обязательно</span>
            </label>
        </div>
        <div class="submitBtn">
  ...
  methods: {
    async submitHandler() {
      this.v$.$touch()
        ...
        await axios.post('api/auth/reg/phone/sms-auth/check/', {
            phone: localStorage.getItem('phone'),
            checkCode: this.checkCode,
        }).then(
            response => {
                console.log(response)
                this.$router.push('/registration/reguser')
            }
        ).catch(
            error => {
                console.log(error)
